...
type CompilationGlobals = DiagnosticsLogger * BuildPhase
...
type NodeCodeStateMachineData<'T> =
    ...
    [<DefaultValue(false)>]
    val mutable CompilationGlobals: CompilationGlobals
...
/// Represents the runtime continuation of a NodeCode state machine created dynamically
...
/// A special compiler-recognised delegate type for specifying blocks of NodeCode code with access to the state machine
...
type NodeCodeBuilderBase() =
    ...
    /// <summary>Creates a NodeCode that runs generator</summary>
    ...
    member inline _.Combine
        ...
        ResumableCode.Combine(
            NodeCodeCode(fun sm ->
                sm.Data.ThrowIfCancellationRequested()
                let result = task1.Invoke(&sm)
                sm.Data.CompilationGlobals <- captureGlobals()
                result
            ),
    ...
    member inline _.While
        ...
        ResumableCode.While(
            ...
            NodeCodeCode(fun sm ->
                sm.Data.ThrowIfCancellationRequested()
                let result = computation.Invoke(&sm)
                sm.Data.CompilationGlobals <- captureGlobals()
                result
            )
        )
    ...
    member inline _.For
        ...
        ResumableCode.For(
            ...
            fun item ->
                NodeCodeCode(fun sm ->
                    sm.Data.ThrowIfCancellationRequested()
                    let result = (body item).Invoke(&sm)
                    sm.Data.CompilationGlobals <- captureGlobals()
                    result
                )
        )
...
/// Contains the node computation expression builder.
[<AutoOpen>]
module NodeCodeBuilder =
    ...
    /// </summary>
    let foregroundNodeCode = NodeCodeBuilder(false)
...
module LowPriority =
    ...
    type NodeCodeBuilderBase with
        ...
        member inline _.Source(awaiter: NodeCode<unit array>) = 
            ...
            NodeCodeCode<'TOverall, _>(
                fun sm ->
                    if __useResumableCode then
                        ...
                        NodeCodeBuilderBase.BindDynamic<'TResult1, 'TResult2, 'Awaiter, 'TOverall>(&sm, getAwaiter, continuation))
        ...
        [<NoEagerConstraintApplication>]
        member inline this.ReturnFrom<'TResult1, 'TResult2, 'Awaiter, 'TOverall when Awaiter<'Awaiter, 'TResult1>> (getAwaiter: 'Awaiter) : NodeCodeCode<_, _> =
...
let setThreadStaticCompilationGlobals (diagnosticsLogger, buildPhase) =
...
module HighPriority =
        ...
        let computation =
            ...
            | _ ->
                async {
                    ...
                    setThreadStaticCompilationGlobals globals
                    return! computation
                }
    ...
    type Control.Async with
        ...
        /// its result.</summary>
        static member inline AwaitNodeCode([<InlineIfLambda>] t: NodeCode<'T>) =
            async {
        ...
        /// <summary>Return an asynchronous computation that will wait for the given task to complete and return
        /// its result.</summary>
        static member inline AwaitNodeCode([<InlineIfLambda>] t: NodeCode) =
            async {
                let! ct = Async.CancellationToken
    ...
    type NodeCodeBuilderBase with
        ...
        member inline this.Source(computation: Async<'TResult1>) =
            this.Source(fun ct -> Async.AsNodeCode(computation) ct)
...
module AsyncExtenions =
    type Control.AsyncBuilder with
        ...
        member inline this.Bind([<InlineIfLambda>] t: NodeCode<'T>, [<InlineIfLambda>] binder: ('T -> Async<'U>)) : Async<'U> =
        ...
        member inline this.Bind([<InlineIfLambda>] t: NodeCode, [<InlineIfLambda>] binder: (unit -> Async<'U>)) : Async<'U> =
            this.Bind(Async.AwaitNodeCode t, binder)
...
module NodeCode =
    ...
    ///     let computation =
    ///         node {
    ///             let! cancellationToken = NodeCode.getCancellationToken()
    ...
    let inline startTask ct ([<InlineIfLambda>] ctask: NodeCode) = ctask ct

    let inline startWithoutCancellation ([<InlineIfLambda>] ctask: NodeCode<_>) = start CancellationToken.None ctask

    let inline startTaskWithoutCancellation ([<InlineIfLambda>] ctask: NodeCode) = startTask CancellationToken.None ctask

    let inline runSynchronously ct ([<InlineIfLambda>] ctask: NodeCode<_>) =
        let task = start ct ctask
        task.GetAwaiter().GetResult()

    let inline runSynchronouslyWithoutCancellation  ([<InlineIfLambda>] ctask: NodeCode<_>) =
        let task = startWithoutCancellation ctask
        task.GetAwaiter().GetResult()

    let inline runTaskSynchronously ct ([<InlineIfLambda>] ctask: NodeCode) =
        let task = startTask ct ctask
        task.GetAwaiter().GetResult()

    let inline runTaskSynchronouslyWithoutCancellation ([<InlineIfLambda>] ctask: NodeCode) =
        let task = startTaskWithoutCancellation ctask
        task.GetAwaiter().GetResult()

    let inline startAsTask ct ([<InlineIfLambda>] ctask: NodeCode<_>) = (start ct ctask) :> Task

    let inline startAsTaskWithoutCancellation ([<InlineIfLambda>] ctask: NodeCode<_>) = (start CancellationToken.None ctask) :> Task

    let inline runAsTaskSynchronously ct ([<InlineIfLambda>] ctask: NodeCode<_>) =
        let task = startAsTask ct ctask
        task.GetAwaiter().GetResult()

    let inline runAsTaskSynchronouslyWithoutCancellation  ([<InlineIfLambda>] ctask: NodeCode<_>) =
        let task = startAsTaskWithoutCancellation ctask
        task.GetAwaiter().GetResult()

    let inline whenAll (tasks: NodeCode<'a> seq) =
        node {
            let! ct = getCancellationToken ()
    ...
    let inline whenAllTasks (tasks: NodeCode seq) =
        node {
            let! ct = getCancellationToken ()
    ...
    let inline sequential (tasks: NodeCode<'a> seq) =
        node {
            let! ct = getCancellationToken ()
    ...
    let inline ignore ([<InlineIfLambda>] ctask: NodeCode<_>) = toUnit ctask
...
module MergeSourcesExtensions =
    ...
    type NodeCodeBuilderBase with
        ...
        member inline _.MergeSources<'TResult1, 'TResult2, 'Awaiter1, 'Awaiter2
            ...
            node {
                let! ct = NodeCode.getCancellationToken ()
                        ...
                        | _ ->
                            ...
                            Async.StartWithContinuations(
                                async {
                                    ...
                                    return! computation |> NodeCode.start ct |> Async.AwaitTask
                                },
                                (fun res ->
