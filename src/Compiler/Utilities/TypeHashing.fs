...
/// <summary>Produce a conservative structural fingerprint of <c>TType</c>.</summary>
module StructuralHashTypes =
    ...
    and nullnessEquals (n1: Nullness) (n2: Nullness) =
        ...
        | Nullness.Variable v1, Nullness.Variable v2 -> v1.TryEvaluate() = v2.TryEvaluate()
        | _ -> false
...
type TypeStructure(wr: WeakReference<TType>, hashCode: int) =

    member inline this.TryGetType() = 
        match wr.TryGetTarget() with
        | (true, ty) -> ValueSome ty
        | _ -> ValueNone

    static member inline Create(ty: TType) =
        TypeStructure(WeakReference<TType>(ty), StructuralHashTypes.hashTType ty)
    
    override this.GetHashCode () = hashCode

    interface IEquatable<TypeStructure> with
        member this.Equals that =
            match this.TryGetType(), that.TryGetType() with
            | ValueSome ty1, ValueSome ty2 -> ty1 === ty2 || StructuralHashTypes.stampEquals ty1 ty2
            | _ -> false
