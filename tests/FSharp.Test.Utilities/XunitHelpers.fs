...
type TestRun(sink) =
    ...
    let rewriteTestMethod (testCase : IXunitTestCase) : ITestMethod =
    ...
    let canRunCollectionInParallel (testCase : IXunitTestCase) =
    ...
    let rewrite (testCase : IXunitTestCase) : IXunitTestCase =
        let testMethod = if canRunCollectionInParallel testCase then rewriteTestMethod testCase else testCase.TestMethod
