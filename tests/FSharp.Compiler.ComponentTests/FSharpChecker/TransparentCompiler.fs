...
let ``File is not checked twice`` () =
    ...
    Assert.Equal<JobEvent list>([Requested; Started; Weakened; Finished], intermediateTypeChecks["FileFirst.fs"])
    Assert.Equal<JobEvent list>([Requested; Started; Weakened; Finished], intermediateTypeChecks["FileThird.fs"])
...
let ``If a file is checked as a dependency it's not re-checked later`` () =
    ...
    Assert.Equal<JobEvent list>([ Requested; Started; Weakened; Finished; Requested], intermediateTypeChecks["FileThird.fs"])
